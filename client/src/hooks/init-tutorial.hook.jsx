...
export default function useInitTutorial() {
	...
	async function initTutorial() {
		if (!Object.keys(tutorialSteps).length) {
			...
			tutorialSteps["search-type"].setValueFunction = () =>
				setSearchType("tradeName");
			tutorialSteps.search.setValueFunction = setSearchValue.bind(
				null,
				"napro"
			);
			tutorialSteps.search.processFunction = getDrugs.bind(
				null,
				"napro",
				"tradeName"
			);
			tutorialSteps["similar-drugs"].processFunction = getDrugs.bind(
				null,
				"NAPROXEN",
				"genericName"
			);
			tutorialSteps["alternative-drugs"].processFunction = getDrugs.bind(
				null,
				"NSAID.PROPIONIC ACID DERIVATIVES",
				"pharmacology"
			);
			tutorialSteps["second-search"].setValueFunction = setSearchValue.bind(
				null,
				"marev"
			);
			tutorialSteps["second-search"].processFunction = getDrugs.bind(
				null,
				"marev",
				"tradeName"
			);
			tutorialSteps["end-tutorial"].setValueFunction = () =>
				document.querySelector(".theme-mode").click();
		...
		tutorialSteps["image-upload"].processFunction = async () => {
			const file = await fetchImageAsFile("https://i.ibb.co/9sz6Q1T/932.jpg");
			await addImage(file);
		};
	}
